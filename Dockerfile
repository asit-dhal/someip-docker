...
RUN cd /root/vsomeip-build/capicxx-dbus-runtime && \
	rm -rf build && mkdir -p build && cd build && \
	cmake .. && \
	make && make install
...
RUN wget http://docs.projects.genivi.org/yamaica-update-site/CommonAPI/generator/3.1/3.1.3/commonapi_dbus_generator.zip && \
...
RUN wget https://github.com/GENIVI/capicxx-someip-tools/releases/download/3.1.12.2/commonapi_someip_generator.zip && \
